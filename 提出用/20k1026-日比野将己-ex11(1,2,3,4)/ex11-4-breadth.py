...
class MazeGame:
    ...
    def is_valid(self, x, y):
        ...
        if self.maze.floormap[y][x] == 0:
            return True

    def neighbors(self, i, j):
        x = [(i, j - 1), (i - 1, j), (i + 1, j), (i, j + 1)]
        value = [v for v in x if self.is_valid(v[0], v[1])]
        print(value)
        return value

    def bradth_search(self):
        queue = []
        for i in range(int(self.maze.height)):  # 〇行目
            for j in range(int(self.maze.width)):  # 〇列目
                if self.maze.floormap[i][j] == 5:
                    queue.append((j, i))
        while True:
            if self.maze.floormap[9][7] == 5:
                self.goal = True
                break
            x = queue.pop(0)
            self.maze.floormap[x[1]][x[0]] = 4

            if self.maze.floormap[x[1]][x[0]] != 3:
                next = self.neighbors(x[0], x[1])

                for i in next:
                    queue.append(i)
                    self.maze.floormap[i[1]][i[0]] = 5
                    self.redraw()  # 再描写
                    tk.update()
                    time.sleep(0.75)
