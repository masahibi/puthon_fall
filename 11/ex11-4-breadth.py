...
class MazeGame:
    ...
    def breadth_search(self):  # 幅優先探索メソッド    p252～255（教科書を参考に死ぬ気でググるしかない）
        queue = []  # 移動先リスト
        for i in range(int(self.maze.height)):  # 〇行目
            for j in range(int(self.maze.width)):  # 〇列目
                if self.maze.floormap[i][j] == 5:  # もし自分初期位置なら
                    queue.append((j, i))  # そのインデックス組を加える
        while True:
            x = queue.pop(0)  # queueから最初の組を取り出す
            ...
            if self.maze.floormap[x[1] + 1][x[0]] == 3:  # もしゴールの１つ手前を探索したら
                ...
                for i in next:  # 1つのインデックス組（探索先）
                    queue.append(i)  # queueに追加
                    print(queue)
                    self.maze.floormap[i[1]][i[0]] = 5  # その移動先を自分にする
                    self.redraw()  # 再描写
                    tk.update()  # 再描写
                    time.sleep(0.5)  # 0.5秒停止
                    self.maze.floormap[i[1]][i[0]] = 4  # いた場所を探索済みにする
