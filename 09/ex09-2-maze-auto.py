...
maze = Maze()
with open(r"maze.txt", mode="w") as file:    # ファイルの書き込み
    file.write(f"{MAZE_WIDTH},{MAZE_HEIGHT}\n")    # １行目の情報を書き込む（文字列注意）
    for x in range(MAZE_WIDTH):    # １１回
        for y in range(MAZE_HEIGHT):    # ９回
            file.write(f"{maze.maze[x][y]}")    # 入れごリストの要素を書き込み
        file.write("\n")    # 1行ごとに改行
